...
void stl_generate_shared_vertices(stl_file *stl, indexed_triangle_set &its)
{
    ...
    for (uint32_t facet_idx = 0; facet_idx < stl->stats.number_of_facets; ++ facet_idx) {
        for (int j = 0; j < VERTICES_PER_FACET; ++ j) {
            ...
            int  vnot      			= (j + VERTICES_PER_FACET - 1) % VERTICES_PER_FACET;
            // Increase the 
            ++ fan_traversal_stamp;
            for (;;) {
            ...
            // Vertex index in facet_in_fan_idx being pivoted around, assigned a shared vertex index.
                ...
                if (vnot >= VERTICES_PER_FACET) {
                    ...
                    if (! edge_direction) {
                        pivot_vertex = (vnot + VERTICES_PER_FACET - 1) % VERTICES_PER_FACET;
...
bool its_write_off(const indexed_triangle_set &its, const char *file)
{
    ...
    for (int i = 0; i < its.vertices.size(); ++ i)
        fprintf(fp, "\t%f %f %f\n", its.vertices[i](X_AXIS), its.vertices[i](Y_AXIS), its.vertices[i](Z_AXIS));
    for (uint32_t i = 0; i < its.indices.size(); ++ i)
        fprintf(fp, "\t3 %d %d %d\n", its.indices[i][kV0], its.indices[i][kV1], its.indices[i][kV2]);
    fclose(fp);
    return true;
}
...
bool its_write_obj(const indexed_triangle_set &its, const char *file)
{
    ...
    for (size_t i = 0; i < its.vertices.size() && write_ok; ++ i)
        write_ok = fprintf(fp, "v %f %f %f\n", its.vertices[i](X_AXIS), its.vertices[i](Y_AXIS), its.vertices[i](Z_AXIS)) >= 0;
    for (size_t i = 0; i < its.indices.size() && write_ok; ++ i)
        write_ok = fprintf(fp, "f %d %d %d\n", its.indices[i][kV0]+1, its.indices[i][kV1]+1, its.indices[i][kV2]+1) >= 0;
...
bool its_write_obj(const indexed_triangle_set& its, const std::vector<obj_color> &color, const char* file)
{
    ...
    for (size_t i = 0; i < its.vertices.size() && write_ok; ++i)
        write_ok = fprintf(fp, "v %f %f %f %f %f %f\n", 
            its.vertices[i](X_AXIS), 
            its.vertices[i](Y_AXIS), 
            its.vertices[i](Z_AXIS),
            color[i](X_AXIS), 
            color[i](Y_AXIS), 
            color[i](Z_AXIS)) >= 0;
    for (size_t i = 0; i < its.indices.size() && write_ok; ++i)
    ...
    for (int facet_idx = 0; facet_idx < static_cast<int>(stl->stats.number_of_facets); ++ facet_idx) {
        ...
        for (int nbr_idx = 0; nbr_idx < VERTICES_PER_FACET; ++ nbr_idx) {
            ...
            if (nbr_face != -1) {
                ...
                assert(vnot_back % VERTICES_PER_FACET == (nbr_idx + VERTICES_PER_FACET - 1) % VERTICES_PER_FACET);
                if (vertices != nullptr) {
                    // Has shared vertices.
                    if (nbr_vnot < VERTICES_PER_FACET) {
                        ...
                        assert((its.indices[nbr_face][(nbr_vnot + 1) % VERTICES_PER_FACET] == vertices[(nbr_idx + 1) % VERTICES_PER_FACET] && its.indices[nbr_face][(nbr_vnot + VERTICES_PER_FACET - 1) % VERTICES_PER_FACET] == vertices[nbr_idx]));
                    } else {
                        ...
                        assert((its.indices[nbr_face][(nbr_vnot + VERTICES_PER_FACET - 1) % VERTICES_PER_FACET] == vertices[(nbr_idx + 1) % VERTICES_PER_FACET] && its.indices[nbr_face][(nbr_vnot + 1) % VERTICES_PER_FACET] == vertices[nbr_idx]));
                    }
                }
            }
        }
