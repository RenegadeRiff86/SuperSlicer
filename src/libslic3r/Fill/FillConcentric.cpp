    ...
    for (size_t i = 0; i < expp.size(); ++i) {
        ...
        for (int idx_bunch = 0; idx_bunch < bunch_2_shell_2_loops.size(); idx_bunch++) {
            ...
            if (idx_bunch < bunch_2_gaps.size() && !bunch_2_gaps[idx_bunch].empty() && params.density >= 1) {
                ...
                min = std::max(min, double(Flow::new_from_spacing(static_cast<float>(EPSILON), static_cast<float>(params.flow.nozzle_diameter()), static_cast<float>(params.flow.height()),
                    static_cast<float>(params.flow.spacing_ratio()), false).scaled_width()));
