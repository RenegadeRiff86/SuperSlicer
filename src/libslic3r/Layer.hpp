...
public:
    ...
    [[nodiscard]] ExtrusionEntityCollection&        set_perimeters() { return m_perimeters; }
...
protected:
    ...
    friend struct std::default_delete<LayerRegion>;

public:
    // Public construction allows std::make_unique; the protected destructor remains mediated by std::default_delete.
...
private:
    ...
    // Note: in SuperSlicer layer_needs_raw_backup is always true, so every layer keeps this cache
    // (upstream only backs it up for multi-region / elephant-foot layers); the conditional
    // backup machinery could therefore be simplified away.
    ExPolygons                  m_raw_slices;

// public only so the unit tests can reach m_slices (it would otherwise be private)
public:
    // collection of surfaces generated by slicing the original geometry
    // divided by type top/bottom/internal
...
public:
    ...
    // Note: raw slices are kept on every layer in SuperSlicer (layer_needs_raw_backup is always true),
    // so this conditional backup/restore could be simplified.
...
protected:
    friend class PrintObject;
    friend struct std::default_delete<Layer>;
