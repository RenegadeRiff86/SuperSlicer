...
namespace Slic3r {
    ...
    void MillingPostProcess::getExtrusionLoop(const Layer* layer, Polygon& poly, Polylines& entrypoints, ExtrusionEntityCollection& out_coll) {
        ...
        if (!entrypoints.empty() && best_polyline.size() > 3 ) {
            ...
            for (int32_t idx = 0; idx < int32_t(poly.points.size()); idx++) {
            ...
            if (first_point_idx > -1) {
                ...
                for (int32_t idx = first_point_idx +1; idx < int32_t(poly.points.size()); idx++) {
                ...
                if (second_point_idx == -1) {
                    ...
                    for (int32_t idx = 0; idx < int32_t(best_polyline.points.size()); idx++) {
                ...
                for (int32_t idx = first_point_idx; idx < int32_t(poly.points.size()); idx++) {
