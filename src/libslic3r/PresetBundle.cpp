    ...
    physical_printers(PhysicalPrinter::printer_options(), this)
{
    ...
    for (size_t i = 0; i < 2; ++ i) {
        ...
        for (const char *key : { 
            ...
            // Note: the following keys are only created here for compatibility, to be able to parse legacy Printer profiles.
...
DynamicPrintConfig PresetBundle::full_config_secure() const
{
    ...
    // Note: legacy -- the keys should not be there after conversion to a Physical Printer profile; erase them defensively.
