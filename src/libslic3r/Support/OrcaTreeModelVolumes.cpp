...
#include "OrcaTreeModelVolumes.hpp"
    ...
    m_machine_border{ calculateMachineBorderCollision(build_volume.polygon()) }
{
    ...
    for (size_t mesh_idx = 0; mesh_idx < storage.meshes.size(); ++ mesh_idx) {
        ...
        for (size_t idx = 0; idx < m_layer_outlines.size(); ++ idx)
            if (OrcaTreeSupport::OrcaTreeSupportSettings(m_layer_outlines[idx].first) == OrcaTreeSupport::OrcaTreeSupportSettings(mesh.settings)) {
                added = true;
...
#else
    {
        ...
        OrcaTreeSupportMeshGroupSettings mesh_settings(print_object);
        const OrcaTreeSupportSettings config{ mesh_settings, print_object.slicing_parameters() };
...
void OrcaTreeModelVolumes::precalculate(const PrintObject& print_object, const coord_t max_layer, std::function<void()> throw_on_cancel)
{
    ...
    OrcaTreeSupportSettings config(m_layer_outlines[m_current_outline_idx].first, print_object.slicing_parameters());
...
const Polygons& OrcaTreeModelVolumes::getCollision(const coord_t orig_radius, LayerIndex layer_idx, bool min_xy_dist) const
{
...
std::optional<std::pair<coord_t, std::reference_wrapper<const Polygons>>> OrcaTreeModelVolumes::get_collision_lower_bound_area(LayerIndex layer_id, coord_t max_radius) const
{
...
const Polygons& OrcaTreeModelVolumes::getCollisionHolefree(coord_t radius, LayerIndex layer_idx) const
{
...
const Polygons& OrcaTreeModelVolumes::getAvoidance(const coord_t orig_radius, LayerIndex layer_idx, AvoidanceType type, bool to_model, bool min_xy_dist) const
{
    ...
    const_cast<OrcaTreeModelVolumes*>(this)->calculateAvoidance({ radius, layer_idx }, ! to_model, to_model);
...
const Polygons& OrcaTreeModelVolumes::getPlaceableAreas(const coord_t orig_radius, LayerIndex layer_idx, std::function<void()> throw_on_cancel) const
{
    ...
    const_cast<OrcaTreeModelVolumes*>(this)->calculatePlaceables(radius, layer_idx, throw_on_cancel);
    return getPlaceableAreas(orig_radius, layer_idx, throw_on_cancel);
}

const Polygons& OrcaTreeModelVolumes::getWallRestriction(const coord_t orig_radius, LayerIndex layer_idx, bool min_xy_dist) const
{
    assert(layer_idx > 0);
    if (layer_idx == 0) 
    ...
    const_cast<OrcaTreeModelVolumes*>(this)->calculateWallRestrictions({ radius, layer_idx });
...
void OrcaTreeModelVolumes::calculateCollision(const std::vector<RadiusLayerPair> &keys, std::function<void()> throw_on_cancel)
{
...
void OrcaTreeModelVolumes::calculateCollision(const coord_t radius, const LayerIndex max_layer_idx, std::function<void()> throw_on_cancel)
{
//    assert(radius == this->ceilRadius(radius));
    ...
    for (size_t outline_idx : layer_outline_indices)
        if (const std::vector<Polygons> &outlines = m_layer_outlines[outline_idx].second; ! outlines.empty()) {
            const OrcaTreeSupportMeshGroupSettings  &settings = m_layer_outlines[outline_idx].first;
...
void OrcaTreeModelVolumes::calculateCollisionHolefree(const std::vector<RadiusLayerPair> &keys, std::function<void()> throw_on_cancel)
{
...
void OrcaTreeModelVolumes::calculateAvoidance(const std::vector<RadiusLayerPair> &keys, bool to_build_plate, bool to_model, std::function<void()> throw_on_cancel)
{
...
void OrcaTreeModelVolumes::calculatePlaceables(const std::vector<RadiusLayerPair> &keys, std::function<void()> throw_on_cancel)
{
    tbb::parallel_for(tbb::blocked_range<size_t>(0, keys.size()),
...
void OrcaTreeModelVolumes::calculatePlaceables(const coord_t radius, const LayerIndex max_required_layer, std::function<void()> throw_on_cancel)
{
...
void OrcaTreeModelVolumes::calculateWallRestrictions(const std::vector<RadiusLayerPair> &keys, std::function<void()> throw_on_cancel)
{
...
coord_t OrcaTreeModelVolumes::ceilRadius(const coord_t radius) const
{
    if (radius == 0)
        return 0;
...
void OrcaTreeModelVolumes::RadiusLayerPolygonCache::allocate_layers(size_t num_layers)
{
...
std::vector<std::pair<OrcaTreeModelVolumes::RadiusLayerPair, std::reference_wrapper<const Polygons>>> OrcaTreeModelVolumes::RadiusLayerPolygonCache::sorted() const
{
