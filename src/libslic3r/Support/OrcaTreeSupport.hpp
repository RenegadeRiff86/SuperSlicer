#ifndef ORCA_TREESUPPORT_H
#define ORCA_TREESUPPORT_H
 ...
 */
class OrcaTreeSupportData
{
public:
    OrcaTreeSupportData() = default;
    /*!
     * \brief Construct the OrcaTreeSupportData object
     *
     ...
     */
    OrcaTreeSupportData(const PrintObject& object, coordf_t xy_distance, coordf_t radius_sample_resolution);
    ~OrcaTreeSupportData() {
        clear_nodes();
    }

    OrcaTreeSupportData(OrcaTreeSupportData&&) = default;
    OrcaTreeSupportData& operator=(OrcaTreeSupportData&&) = default;

    OrcaTreeSupportData(const OrcaTreeSupportData&) = delete;
    OrcaTreeSupportData& operator=(const OrcaTreeSupportData&) = delete;
...
public:
     ...
     */
    OrcaTreeSupport(PrintObject& object, const SlicingParameters &slicing_params);

    void move_bounds_to_contact_nodes(std::vector<OrcaTreeSupport3D::SupportElements> &move_bounds,
...
private:
    ...
    std::shared_ptr<OrcaTreeSupportData> m_ts_data;
    std::unique_ptr<OrcaTreeSupport3D::OrcaTreeModelVolumes> m_model_volumes;
    PrintObject    *m_object;
    const PrintObjectConfig* m_object_config;
    SlicingParameters        m_slicing_params;
    // Various precomputed support parameters to be shared with external functions.
    FFFSupport::SupportParameters m_support_params;
