...
#ifndef slic3r_OrcaTreeSupportCommon_hpp
#define slic3r_OrcaTreeSupportCommon_hpp
...
struct OrcaTreeSupportMeshGroupSettings {
    OrcaTreeSupportMeshGroupSettings() = default;
    explicit OrcaTreeSupportMeshGroupSettings(const PrintObject &print_object)
    {
    ...
    //    const std::vector<unsigned int>  printing_extruders = print_object.object_extruders();
        ...
        // Support must be enabled and set to Orca tree style.
        assert(config.support_material || config.support_material_enforce_layers > 0);
        assert(config.support_material_style.value == smsOrcaTree);
        ...
        // Keep Orca's minimum feature setting local until SuperSlicer exposes a matching tree option.
        this->min_feature_size          = scale_t(external_perimeter_width * 0.25);
 ...
 */
struct OrcaTreeSupportSettings
{
public:
    OrcaTreeSupportSettings() = default; // required for the definition of the config variable in the OrcaTreeSupportGenerator class.
    explicit OrcaTreeSupportSettings(const OrcaTreeSupportMeshGroupSettings &mesh_group_settings, const SlicingParameters &slicing_params)
          ...
          min_feature_size(mesh_group_settings.min_feature_size)
    {
        ...
        if (OrcaTreeSupportSettings::soluble) {
    ...
    // Has to be static because OrcaTreeSupportConfig will be used in OrcaTreeModelVolumes as this reduces redundancy.
...
//        std::vector<double> support_infill_angles;
     ...
     */
    OrcaTreeSupportMeshGroupSettings settings;
...
public:
    bool operator==(const OrcaTreeSupportSettings& other) const
    {
        return branch_radius == other.branch_radius && tip_layers == other.tip_layers && branch_radius_increase_per_layer == other.branch_radius_increase_per_layer &&
            layer_start_bp_radius == other.layer_start_bp_radius && bp_radius == other.bp_radius && 
               ...
               support_rests_on_model == other.support_rests_on_model && increase_radius_until_layer == other.increase_radius_until_layer && min_dtt_to_model == other.min_dtt_to_model &&
                   max_to_model_radius_increase == other.max_to_model_radius_increase && maximum_move_distance == other.maximum_move_distance &&
                   maximum_move_distance_slow == other.maximum_move_distance_slow && z_distance_bottom_layers == other.z_distance_bottom_layers && support_line_width == other.support_line_width && 
               support_line_spacing == other.support_line_spacing && support_roof_line_width == other.support_roof_line_width &&
               ...
               support_bottom_offset == other.support_bottom_offset && support_wall_count == other.support_wall_count && support_pattern == other.support_pattern && roof_pattern == other.roof_pattern &&
               ...
               increase_radius_until_radius == other.increase_radius_until_radius && support_bottom_layers == other.support_bottom_layers && layer_height == other.layer_height &&
                   z_distance_top_layers == other.z_distance_top_layers && resolution == other.resolution && // Infill generation depends on deviation and resolution.
               ...
               // The infill class now wants the settings object and reads a lot of settings, and as the infill class is used to calculate support roof lines for interface-preference. Not all of these
               // may be required to be identical, but as I am not sure, better safe than sorry
#if 0
...
{ // todo Remove!  ONLY FOR PUBLIC BETA!!
    ...
    if (show)
        MessageBoxA(nullptr, std::string("OrcaTreeSupport_2 MOD detected an error while generating the tree support.\nPlease report this back to me with profile and model.\nRevision 5.0\n" + std::string(message) + "\n" + bugtype).c_str(), 
            "Bug detected!", MB_OK | MB_SYSTEMMODAL | MB_SETFOREGROUND | MB_ICONWARNING);
#endif // TREE_SUPPORT_SHOW_ERRORS_WIN32
}

inline double layer_z(const SlicingParameters &slicing_params, const OrcaTreeSupportSettings &config, const size_t layer_idx)
{
...
inline LayerIndex layer_idx_ceil(const SlicingParameters &slicing_params, const OrcaTreeSupportSettings &config, const double z)
{
    return 
...
inline LayerIndex layer_idx_floor(const SlicingParameters &slicing_params, const OrcaTreeSupportSettings &config, const double z)
{
    return 
        LayerIndex(config.raft_layers.size()) + 
...
#endif // slic3r_OrcaTreeSupportCommon_hpp
