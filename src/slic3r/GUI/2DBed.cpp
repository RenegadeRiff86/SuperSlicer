...
static constexpr int ALTERNATING_GRID_DIVISOR = 2;
static constexpr int GRID_PEN_WIDTH = 2;
static constexpr int GRID_COLOR_CHANNEL = 230;
static constexpr int ORIGIN_MARKER_HALF_SIZE = 15;
...
void Bed_2D::repaint(const std::vector<Vec2d>& shape)
{
    ...
    cw -= (DOUBLE_BORDER * Border);
    ch -= (DOUBLE_BORDER * Border);
    ...
    for (double x = bb.min(0) - fmod(bb.min(0), step) + step; x < bb.max(0); x += step, ++idx) {
        ...
        else if (idx % ALTERNATING_GRID_DIVISOR == 1)
    ...
    dc.SetPen(wxPen(wxColour(GRID_COLOR_CHANNEL, GRID_COLOR_CHANNEL, GRID_COLOR_CHANNEL), 1, wxPENSTYLE_SOLID));
    ...
    dc.SetPen(wxPen(wxColour(GRID_COLOR_CHANNEL, GRID_COLOR_CHANNEL, GRID_COLOR_CHANNEL), GRID_PEN_WIDTH, wxPENSTYLE_SOLID));
    ...
    dc.SetPen(wxPen(wxColour(GRID_COLOR_CHANNEL, GRID_COLOR_CHANNEL, GRID_COLOR_CHANNEL), 3, wxPENSTYLE_SOLID));
    ...
    if (m_pos!= Vec2d(0, 0)) {
        ...
        dc.SetPen(wxPen(wxColour(200, 0, 0), GRID_PEN_WIDTH, wxPENSTYLE_SOLID));
        dc.SetBrush(wxBrush(wxColour(200, 0, 0), wxBRUSHSTYLE_TRANSPARENT));
        dc.DrawCircle(pos_px(0), pos_px(1), 5);

        dc.DrawLine(pos_px(0) - ORIGIN_MARKER_HALF_SIZE, pos_px(1), pos_px(0) + ORIGIN_MARKER_HALF_SIZE, pos_px(1));
        dc.DrawLine(pos_px(0), pos_px(1) - ORIGIN_MARKER_HALF_SIZE, pos_px(0), pos_px(1) + ORIGIN_MARKER_HALF_SIZE);
    }
}
