...
{
    constexpr Eigen::Index spatial_dimensions = 3;
    Vec4i32                viewport(camera.get_viewport().data());

    // Convert our std::vector to Eigen dynamic matrix.
    Eigen::Matrix<double, Eigen::Dynamic, spatial_dimensions, Eigen::DontAlign>
        pts(points.size(), spatial_dimensions);
    for (size_t i = 0; i < points.size(); ++i)
        pts.block<1, spatial_dimensions>(i, 0) = points[i];

    // Get the projections.
    Eigen::Matrix<double, Eigen::Dynamic, spatial_dimensions, Eigen::DontAlign> projections;
