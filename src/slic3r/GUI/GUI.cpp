// |/ Copyright (c) Prusa Research 2016 - 2023 Tomáš Mészáros @tamasmeszaros, Oleksandra Iushchenko @YuSanka, Vojtěch Bubník @bubnikv, Lukáš Matěna @lukasmatena, Lukáš Hejl @hejllukas, David Kocík @kocikdav, Enrico Turri @enricoturri1966, Vojtěch
// Král @vojtechkral
...
static constexpr size_t SUBSTITUTION_PAIR_SIZE = 2;
static constexpr size_t TRAILING_SEPARATOR_LENGTH = 2;
...
static void add_config_substitutions(const ConfigSubstitutions& conf_substitutions, wxString& changes)
{
    ...
    for (const ConfigSubstitution& conf_substitution : conf_substitutions) {
        ...
        case coBools:
            ...
            if (! new_val.empty())
                new_val.erase(new_val.begin() + new_val.size() - TRAILING_SEPARATOR_LENGTH, new_val.end());
            break;
        case coGraph:
...
//		comboCtrl->EnablePopupAnimation(false);
        ...
        assert(items_str.size() % SUBSTITUTION_PAIR_SIZE == 0);

        for (size_t i = 0; i < items_str.size(); i += SUBSTITUTION_PAIR_SIZE) {
            wxString label = from_u8(items_str[i]);
            max_width = std::max(max_width, 60 + popup->GetTextExtent(label).x);
            popup->Append(label);
            popup->Check(i / SUBSTITUTION_PAIR_SIZE, items_str[i + 1] == "1");
        }
