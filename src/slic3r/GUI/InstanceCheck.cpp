...
static constexpr size_t WINDOW_TEXT_CAPACITY = 1000;
...
namespace instance_check_internal
{
    ...
    static BOOL CALLBACK EnumWindowsProc(_In_ HWND   hwnd, _In_ LPARAM lParam)
    {
        ...
        TCHAR 		 wndText[WINDOW_TEXT_CAPACITY];
        TCHAR 		 className[WINDOW_TEXT_CAPACITY];
        int          err;
        err = GetClassName(hwnd, className, WINDOW_TEXT_CAPACITY);
        if (err == 0)
            return true;
        err = GetWindowText(hwnd, wndText, WINDOW_TEXT_CAPACITY);
        if (err == 0)
            return true;
...
void OtherInstanceMessageHandler::print_window_info(HWND hwnd)
{
    ...
    TCHAR 		 wndText[WINDOW_TEXT_CAPACITY];
    TCHAR 		 className[WINDOW_TEXT_CAPACITY];
    GetClassName(hwnd, className, WINDOW_TEXT_CAPACITY);
    GetWindowText(hwnd, wndText, WINDOW_TEXT_CAPACITY);
    std::wstring classNameString(className);
    std::wstring wndTextString(wndText);
    HANDLE       handle = GetProp(hwnd, L"Instance_Hash_Minor");
