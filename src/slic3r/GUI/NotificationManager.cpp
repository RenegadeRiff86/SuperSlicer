...
static constexpr int THIRD_DIVISOR = 3;
static constexpr int QUARTER_DIVISOR = 4;
static constexpr size_t TWO_LINE_COUNT = 2;
static constexpr size_t THREE_LINE_COUNT = 3;
static constexpr int POPPED_STYLE_COLOR_COUNT = 3;
static constexpr size_t PREVIOUS_LINE_OFFSET = 2;
static constexpr int PERCENT_DECIMAL_PLACES = 2;
static constexpr size_t TRAILING_DELIMITER_LENGTH = 2;
static constexpr int DOUBLE_BUTTON_GUTTER = 2;
...
namespace {
    ...
    */
    ...
    // Every colour here is an ImVec4 of 0..1 channels, but the draw list wants a packed 32-bit value.
    // The conversion is the same three casts every time - it was written out at four call sites, with
    // twelve copies of the 255 between them. Alpha stays a separate argument because the callers do
    // not agree on it: the hypertext underline scales the colour's own alpha by the fade, while the
    // progress bar uses the fade alone and ignores whatever alpha the colour carries.
    ImU32 to_im_col32(const ImVec4& col, float alpha)
    {
        return IM_COL32(static_cast<int>(col.x * CHANNEL_VALUE_MAX),
                        static_cast<int>(col.y * CHANNEL_VALUE_MAX),
                        static_cast<int>(col.z * CHANNEL_VALUE_MAX),
                        alpha);
    }
...
void NotificationManager::PopNotification::render(GLCanvas3D& canvas, float initial_y, bool move_from_overlay, float overlay_width)
{
    ...
    if (imgui.begin(name, flags)) {
        ...
        if (m_multiline && m_lines_count > THREE_LINE_COUNT)
    ...
    if (fading_pop)
        ImGui::PopStyleColor(POPPED_STYLE_COLOR_COUNT);
}
...
void NotificationManager::PopNotification::count_spaces()
{
    ...
    if (m_data.level == NotificationLevel::ErrorNotificationLevel 
        ...
        m_left_indentation = picture_width + m_line_height / HALF_DIVISOR;
    }
    m_window_width_offset = m_left_indentation + m_line_height * ButtonGutterLines;
    m_window_width = m_line_height * WindowWidthLines;
}
        ...
        if (next_hard_end != std::string::npos && ImGui::CalcTextSize(text.substr(last_end, next_hard_end - last_end).c_str()).x < m_window_width - m_window_width_offset) {
            ...
            if (ImGui::CalcTextSize(text.substr(last_end).c_str()).x >= m_window_width - m_window_width_offset) {
                ...
                if (ImGui::CalcTextSize(text.substr(last_end, next_space - last_end).c_str()).x > m_window_width - m_window_width_offset ||
                    ImGui::CalcTextSize(text.substr(last_end, next_space - last_end).c_str()).x < (m_window_width - m_window_width_offset) / QUARTER_DIVISOR * 3
                    ) {
...
void NotificationManager::PopNotification::render_text(ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    ...
    int		last_end = 0;
    float	starting_y = (m_lines_count == TWO_LINE_COUNT ? win_size_y / HALF_DIVISOR - m_line_height : (m_lines_count == 1 ? win_size_y / HALF_DIVISOR - m_line_height / HALF_DIVISOR : m_line_height / HALF_DIVISOR));
    ...
    for (size_t i = 0; i < (m_multiline ? m_endlines.size() : std::min(m_endlines.size(), m_normal_lines_count)); i++) {
        ...
        if (m_endlines.size() > i && m_text1.size() >= m_endlines[i]) {
            if (i == m_normal_lines_count - 1 && m_endlines.size() > m_normal_lines_count && !m_multiline) {
                ...
                assert(m_normal_lines_count - PREVIOUS_LINE_OFFSET >= 0);
                line = m_text1.substr(m_endlines[m_normal_lines_count - PREVIOUS_LINE_OFFSET] + (m_text1[m_endlines[m_normal_lines_count - PREVIOUS_LINE_OFFSET]] == '\n' || m_text1[m_endlines[m_normal_lines_count - PREVIOUS_LINE_OFFSET]] == ' ' ? 1 : 0),
  m_endlines[m_normal_lines_count - 1] - m_endlines[m_normal_lines_count - PREVIOUS_LINE_OFFSET] - (m_text1[m_endlines[m_normal_lines_count - PREVIOUS_LINE_OFFSET]] == '\n' || m_text1[m_endlines[m_normal_lines_count - PREVIOUS_LINE_OFFSET]] == ' ' ? 1 : 0));
...
void NotificationManager::PopNotification::render_hypertext(ImGuiWrapper& imgui, const float text_x, const float text_y, const std::string& text, bool more)
{
    ...
    lineEnd.y -= PREVIOUS_LINE_OFFSET;
    ImVec2 lineStart = lineEnd;
    lineStart.x = ImGui::GetItemRectMin().x;
    ImGui::GetWindowDrawList()->AddLine(lineStart, lineEnd, to_im_col32(blue_color_vec,
        static_cast<int>(blue_color_vec.w * CHANNEL_VALUE_MAX * (m_state == EState::FadingOut ? m_current_fade_opacity : 1.f))));

}

bool NotificationManager::PopNotification::is_close_slot_hovered(const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y) const
{
    return ImGui::IsMouseHoveringRect(ImVec2(win_pos_x - win_size_x / HoverBandWindowDivisor, win_pos_y),
                                      ImVec2(win_pos_x, win_pos_y + win_size_y - minimize_band_height()),
                                      true);
}

bool NotificationManager::PopNotification::is_action_slot_hovered(const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y) const
{
    return ImGui::IsMouseHoveringRect(ImVec2(win_pos_x - m_line_height * ActionSlot.glyph_x, win_pos_y),
                                      ImVec2(win_pos_x - m_line_height * ActionSlotHoverRight, win_pos_y + win_size_y),
                                      true);
}

void NotificationManager::PopNotification::push_button_style_colors()
{
    ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(.0f, .0f, .0f, .0f));
    ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(.0f, .0f, .0f, .0f));
...
void NotificationManager::PopNotification::pop_button_style_colors()
{
    ImGui::PopStyleColor(ButtonStyleColorCount);
}

void NotificationManager::PopNotification::render_slot_button(ImGuiWrapper& imgui, const ButtonSlot& slot, const wxString& glyph, const std::string& glyph_utf8, const float win_size_x, const float win_size_y, const std::function<void()>& on_click)
{
    ImVec2 button_pic_size = ImGui::CalcTextSize(glyph_utf8.c_str());
    ImVec2 button_size(button_pic_size.x * ButtonGlyphScale, button_pic_size.y * ButtonGlyphScale);
    ImGui::SetCursorPosX(win_size_x - m_line_height * slot.glyph_x);
    ImGui::SetCursorPosY(win_size_y / HALF_DIVISOR - button_size.y);
    if (imgui.button(glyph, button_size.x, button_size.y))
    {
        on_click();
    }

    //invisible large button
    ImGui::SetCursorPosX(win_size_x - m_line_height * slot.hitbox_x);
    ImGui::SetCursorPosY(0);
    if (imgui.button(slot.hitbox_label, m_line_height * slot.hitbox_width, slot.clears_minimize ? win_size_y - minimize_band_height() : win_size_y))
    {
        on_click();
    }
}
...
{
    push_button_style_colors();

    std::string button_text;
    button_text = ImGui::CloseNotifButton;
    
    if (is_close_slot_hovered(win_size_x, win_size_y, win_pos_x, win_pos_y))
    {
        button_text = ImGui::CloseNotifHoverButton;
    }
    render_slot_button(imgui, CloseSlot, button_text.c_str(), button_text, win_size_x, win_size_y, [this]() { close(); });
    pop_button_style_colors();
}
...
void NotificationManager::PopNotification::render_left_sign(ImGuiWrapper& imgui)
{
    if (m_data.level == NotificationLevel::ErrorNotificationLevel || m_data.level == NotificationLevel::WarningNotificationLevel) {
        std::string text;
        text = (m_data.level == NotificationLevel::ErrorNotificationLevel ? ImGui::ErrorMarker : ImGui::WarningMarker);
        ImGui::SetCursorPosX(m_line_height / THIRD_DIVISOR);
        ImGui::SetCursorPosY(m_window_height / HALF_DIVISOR - m_line_height);
        imgui.text(text.c_str());
...
void NotificationManager::PopNotification::render_minimize_button(ImGuiWrapper& imgui, const float win_pos_x, const float win_pos_y)
{
    ...
    if (ImGui::IsMouseHoveringRect(ImVec2(win_pos_x - m_window_width / HoverBandWindowDivisor, win_pos_y + m_window_height - MinimizeBandLines * m_line_height + 1),
...
void NotificationManager::ExportFinishedNotification::count_spaces()
{
    ...
    m_window_width_offset = m_left_indentation + m_line_height * (m_to_removable ? DOUBLE_BUTTON_GUTTER * ButtonGutterLines : ButtonGutterLines);
    m_window_width = m_line_height * WindowWidthLines;
}
...
void NotificationManager::ExportFinishedNotification::render_text(ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    ...
    size_t last_end = 0;
    float starting_y = m_line_height / HALF_DIVISOR;//10;
    float shift_y = m_line_height;// -m_line_height / HALF_DIVISOR0;
...
void NotificationManager::ExportFinishedNotification::render_eject_button(ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    ...
    if (is_action_slot_hovered(win_size_x, win_size_y, win_pos_x, win_pos_y))
    {
    ...
    render_slot_button(imgui, ActionSlot, button_text.c_str(), button_text, win_size_x, win_size_y, [this]() {
...
void NotificationManager::ProgressBarNotification::init()
{
    ...
    if(m_lines_count >= TWO_LINE_COUNT) {
        //m_lines_count = THREE_LINE_COUNT;
        m_multiline = true;
        while (m_endlines.size() < THREE_LINE_COUNT)
            m_endlines.push_back(m_endlines.back());
    } else {
        m_lines_count = TWO_LINE_COUNT;
        m_endlines.push_back(m_endlines.back());
        m_multiline = false;
    }
...
void NotificationManager::ProgressBarNotification::render_text(ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    ...
    if (m_multiline) {
        ...
        ImGui::SetCursorPosY(m_line_height / QUARTER_DIVISOR);
        imgui.text(m_text1.substr(0, m_endlines[0]).c_str());
        ImGui::SetCursorPosX(m_left_indentation);
        ImGui::SetCursorPosY(m_line_height + m_line_height / QUARTER_DIVISOR);
        ...
        ImGui::SetCursorPosX(m_left_indentation);
        ImGui::SetCursorPosY(/*win_size_y / HALF_DIVISOR - win_size_y / ROW_OFFSET_DIVISOR -*/ m_line_height / QUARTER_DIVISOR);
        imgui.text(m_text1.substr(0, m_endlines[0]).c_str());
        if (m_has_cancel_button)
            render_cancel_button(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y);
        render_bar(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y);
...
void NotificationManager::ProgressBarNotification::render_bar(ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y, float y_indentation, float percent, bool render)
{
    ...
    ImVec2 lineEnd = ImVec2(win_pos_x - m_window_width_offset, win_pos_y + win_size_y / HALF_DIVISOR + y_indentation);
    ImVec2 lineStart = ImVec2(win_pos_x - win_size_x + m_left_indentation, win_pos_y + win_size_y / HALF_DIVISOR + y_indentation);
    ImVec2 midPoint = ImVec2(lineStart.x + (lineEnd.x - lineStart.x) * percent, lineStart.y);
    ImGui::GetWindowDrawList()->AddLine(lineStart, lineEnd, to_im_col32(gray_color, m_current_fade_opacity * CHANNEL_VALUE_MAX), m_line_height * 0.2f);
    ImGui::GetWindowDrawList()->AddLine(lineStart, midPoint, to_im_col32(orange_color, m_current_fade_opacity * CHANNEL_VALUE_MAX), m_line_height * 0.2f);
    if (render) {
        std::string text;
        std::stringstream stream;
        stream << std::fixed << std::setprecision(PERCENT_DECIMAL_PLACES) << static_cast<int>(percent * PERCENT_PER_UNIT) << "%";
        text = stream.str();
        ImGui::SetCursorPosX(m_left_indentation);
        ImGui::SetCursorPosY(win_size_y / HALF_DIVISOR + win_size_y / ROW_OFFSET_DIVISOR + y_indentation - m_line_height / HALF_DIVISOR);
        imgui.text(text.c_str());
    }
}
...
{
    render_bar(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y, (m_multiline ? m_line_height / HALF_DIVISOR : 0), m_percentage, m_render_percentage);
}
...
void NotificationManager::ProgressBarWithCancelNotification::render_cancel_button_inner(ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    ...
    render_slot_button(imgui, CloseSlot, button_text.c_str(), button_text, win_size_x, win_size_y, [this]() { on_cancel_button(); });
    pop_button_style_colors();
}
...
void NotificationManager::ProgressBarWithCancelNotification::render_bar(ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    ...
    if (m_percentage < 0.f) {
        ...
        std::stringstream stream;
        stream << std::fixed << std::setprecision(PERCENT_DECIMAL_PLACES) << static_cast<int>(m_percentage * PERCENT_PER_UNIT) << "%";
        text = stream.str();
    }
    ImGui::SetCursorPosX(m_left_indentation);
    ImGui::SetCursorPosY(win_size_y / HALF_DIVISOR + win_size_y / ROW_OFFSET_DIVISOR - (m_multiline ? 0 : m_line_height / QUARTER_DIVISOR));
    imgui.text(text.c_str());
}
...
void NotificationManager::URLDownloadNotification::render_pause_button_inner(ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    ...
    render_slot_button(imgui, ActionSlot, button_text.c_str(), boost::nowide::narrow(button_text), win_size_x, win_size_y,
        [this]() { trigger_user_action_callback(m_download_paused ? DownloaderUserAction::DownloadUserContinued : DownloaderUserAction::DownloadUserPaused); });
    pop_button_style_colors();
}
...
void NotificationManager::URLDownloadNotification::render_open_button_inner(ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    ...
    render_slot_button(imgui, ActionSlot, button_text.c_str(), boost::nowide::narrow(button_text), win_size_x, win_size_y,
        [this]() { trigger_user_action_callback(DownloaderUserAction::DownloadUserOpenedFolder); });
    pop_button_style_colors();
}
...
void NotificationManager::URLDownloadNotification::render_cancel_button_inner(ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    ...
    render_slot_button(imgui, CloseSlot, button_text.c_str(), button_text, win_size_x, win_size_y,
        [this]() { trigger_user_action_callback(DownloaderUserAction::DownloadUserCanceled); });
    pop_button_style_colors();
}
...
void NotificationManager::URLDownloadNotification::render_bar(ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    auto shorten_to_line = [this](const std::string& text, bool dots) -> std::string {
        ...
        if (did_shorten && dots) {
            line = line.substr(0, line.length() - TRAILING_DELIMITER_LENGTH);
            line += "...";
        }
        return line;
    };
...
void NotificationManager::URLDownloadNotification::count_spaces()
{
    ...
    m_window_width_offset = m_line_height * (DOUBLE_BUTTON_GUTTER * ButtonGutterLines); 
}
...
void NotificationManager::PrintHostUploadNotification::generate_text()
{
    ...
    if (ImGui::CalcTextSize((line1 + line2).c_str()).x > m_window_width - m_window_width_offset)
        m_normal_lines_count = THREE_LINE_COUNT;
    else
...
void NotificationManager::PrintHostUploadNotification::render_bar(ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    ...
    case Slic3r::GUI::NotificationManager::PrintHostUploadNotification::UploadJobState::PB_PROGRESS:
    {
        ...
        std::stringstream stream;
        stream << std::fixed << std::setprecision(PERCENT_DECIMAL_PLACES) << static_cast<int>(m_percentage * PERCENT_PER_UNIT) << "% - " << uploaded << " of " << m_file_size << "MB uploaded";
        text = stream.str();
        ImGui::SetCursorPosX(m_left_indentation);
        ImGui::SetCursorPosY(win_size_y / HALF_DIVISOR + win_size_y / ROW_OFFSET_DIVISOR - (m_multiline ? 0 : m_line_height / QUARTER_DIVISOR));
        break;
    ...
    case Slic3r::GUI::NotificationManager::PrintHostUploadNotification::UploadJobState::PB_RESOLVING:
        ...
        ImGui::SetCursorPosX(m_left_indentation);
        ImGui::SetCursorPosY(win_size_y / HALF_DIVISOR + win_size_y / ROW_OFFSET_DIVISOR - (m_multiline ? m_line_height / QUARTER_DIVISOR : m_line_height / HALF_DIVISOR));
        break;
...
void NotificationManager::PrintHostUploadNotification::render_cancel_button(ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    ...
    render_slot_button(imgui, CloseSlot, button_text.c_str(), button_text, win_size_x, win_size_y,
        [this]() { wxGetApp().printhost_job_queue().cancel(m_job_id - 1); });
    pop_button_style_colors();
    ...
    ImGui::SetCursorPosY(win_size.y / HALF_DIVISOR - button_size.y);
    if (imgui.button(button_text.c_str(), button_size.x, button_size.y))
    {
...
void NotificationManager::SlicingProgressNotification::render_text(ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    if (m_sp_state == SlicingProgressState::SP_PROGRESS /*|| (m_sp_state == SlicingProgressState::SP_COMPLETED && !m_sidebar_collapsed)*/) {
        ...
        // two lines text (what doesnt fit, wont show), one line bar
            ...
            imgui.text(m_text1.c_str());
            ImGui::SetCursorPosX(m_left_indentation);
            ImGui::SetCursorPosY(m_line_height * 2 + m_line_height / QUARTER_DIVISOR);
            imgui.text(m_text2.c_str());
            if (m_has_cancel_button)
                render_cancel_button(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y);
        ...
        if (m_multiline) {
            ...
            if (m_sidebar_collapsed && m_sp_state == SlicingProgressState::SP_PROGRESS && m_export_possible) {
                ...
                render_hypertext(imgui, m_left_indentation + text_size.x + 4, m_line_height + m_line_height / QUARTER_DIVISOR, m_hypertext);
            }
            if (m_has_cancel_button)
                render_cancel_button(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y);
            render_bar(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y);
            ...
            if (m_sidebar_collapsed && m_sp_state == SlicingProgressState::SP_PROGRESS && m_export_possible) {
                ImVec2 text_size = ImGui::CalcTextSize(line.c_str());
                render_hypertext(imgui, m_left_indentation + text_size.x + 4, m_line_height / QUARTER_DIVISOR, m_hypertext);
            }
            if (m_has_cancel_button)
                render_cancel_button(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y);
            render_bar(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y);
        ...
        */
        ...
        float cursor_y = win_size.y / HALF_DIVISOR - text_size.y / HALF_DIVISOR;
        if (m_sidebar_collapsed && m_has_print_info) {
            x_offset = 20;
            cursor_y = win_size.y / HALF_DIVISOR + win_size.y / ROW_OFFSET_DIVISOR - text_size.y / HALF_DIVISOR;
            ImGui::SetCursorPosX(x_offset);
