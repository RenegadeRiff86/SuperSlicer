...
static constexpr int DRIVE_MASK_PREFIX_BITS = 2;
static constexpr size_t MIN_USB_DESCRIPTOR_SIZE = 2;
static constexpr size_t USB_WCHAR_BYTES = 2;
static constexpr int DISK_ENUMERATION_DELAY_SECONDS = 2;
...
std::vector<DriveData> RemovableDriveManager::search_for_removable_drives() const
{
    ...
    // Skip A and B drives.
    drives_mask >>= DRIVE_MASK_PREFIX_BITS;
...
)
{
    ...
    if (nbytes_returned < MIN_USB_DESCRIPTOR_SIZE) {
        return NULL;
    }
    ...
    if (string_desc->bLength % USB_WCHAR_BYTES != 0) {
        return NULL;
    }
...
)
{
    ...
    for (tail = string_desc_node_head; tail != NULL; tail = tail->Next) {
        if (tail->DescriptorIndex == descriptor_index) {
            ...
            for(int i = 0; i < tail->StringDescriptor->bLength / USB_WCHAR_BYTES - 1; i++) {
...
bool is_card_reader(HDEVINFO h_dev_info, SP_DEVINFO_DATA& spdd)
{
    ...
    num_language_IDs			= (supported_languages_string->StringDescriptor->bLength - USB_WCHAR_BYTES) / 2;
...
void eject_alt(const std::string& path, wxEvtHandler* callback_evt_handler, DriveData drive_data)
{
...
#else
            m_thread_stop_condition.wait_for(lck, std::chrono::seconds(DISK_ENUMERATION_DELAY_SECONDS), [this]{ return m_stop; });
#endif
        }
        if (m_stop)
