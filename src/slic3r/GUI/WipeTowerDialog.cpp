...
namespace {
constexpr int GTK_ITEM_WIDTH_UNITS = 10;
constexpr int DIALOG_SPACING = 10;
constexpr int MULTIPLIER_MIN = 10;
constexpr int GRID_VERTICAL_GAP = 10;
constexpr int SMALL_SPACING = 5;
constexpr double RAMMING_TIME_MAX = 5.0;
constexpr int LARGE_SPACING = 20;
constexpr int DECIMAL_DIGITS = 2;
constexpr int PARAMETER_COLUMN_COUNT = 2;
constexpr int EXTRUDER_PAIR_SIZE = 2;
constexpr float HALF_DIVISOR = 2.0f;
}

int scale(const int val) { return val * wxGetApp().em_unit(); }
#ifdef __WXGTK3__
int ITEM_WIDTH() { return scale(GTK_ITEM_WIDTH_UNITS); }
#else
...
: wxDialog(parent, wxID_ANY, _(L("Ramming customization")), wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE/* | wxRESIZE_BORDER*/)
{
    ...
    main_sizer->Add(m_panel_ramming, 1, wxEXPAND | wxTOP | wxLEFT | wxRIGHT, SMALL_SPACING);
    ...
    main_sizer->Add(buttons, 0, wxALIGN_CENTER_HORIZONTAL | wxTOP | wxBOTTOM, DIALOG_SPACING);
    SetSizer(main_sizer);
...
: wxPanel(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize/*,wxPoint(50,50), wxSize(800,350),wxBORDER_RAISED*/)
{
...
#endif
    sizer_chart->Add(m_chart, 0, wxALL, SMALL_SPACING);

    m_widget_time						= new ::SpinInputDouble(this,"", wxEmptyString, wxDefaultPosition, wxSize(ITEM_WIDTH(), -1), style, 0., RAMMING_TIME_MAX, 3., 0.5);
    m_widget_time->SetDigits(DECIMAL_DIGITS);
    m_widget_volume							  = new ::SpinInput(this,"",wxEmptyString,wxDefaultPosition,wxSize(ITEM_WIDTH(), -1),style,0,10000,0);
    m_widget_ramming_line_width_multiplicator = new ::SpinInput(this,"",wxEmptyString,wxDefaultPosition,wxSize(ITEM_WIDTH(), -1),style, MULTIPLIER_MIN, 200, 100);
    m_widget_ramming_step_multiplicator		  = new ::SpinInput(this,"",wxEmptyString,wxDefaultPosition,wxSize(ITEM_WIDTH(), -1),style, MULTIPLIER_MIN, 200, 100);
    ...
    auto gsizer_param = new wxFlexGridSizer(PARAMETER_COLUMN_COUNT, SMALL_SPACING, 15);
    ...
    gsizer_param->AddSpacer(LARGE_SPACING);
    gsizer_param->AddSpacer(LARGE_SPACING);
    ...
    sizer_param->Add(gsizer_param, 0, wxTOP, scale(DIALOG_SPACING));
    ...
    sizer->Add(sizer_chart, 0, wxALL, SMALL_SPACING);
    sizer->Add(sizer_param, 0, wxALL, DIALOG_SPACING);
...
: wxDialog(parent, wxID_ANY, _(L("Wipe tower - Purging volume adjustment")), wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE/* | wxRESIZE_BORDER*/)
{
    ...
    main_sizer->Add(m_panel_wiping, 0, wxEXPAND | wxALL, SMALL_SPACING);
    main_sizer->Add(widget_button, 0, wxALIGN_CENTER_HORIZONTAL | wxCENTER | wxBOTTOM, SMALL_SPACING);
    auto buttons = CreateStdDialogButtonSizer(wxOK | wxCANCEL);
    wxGetApp().SetWindowVariantForButton(buttons->GetAffirmativeButton());
    wxGetApp().SetWindowVariantForButton(buttons->GetCancelButton());
    main_sizer->Add(buttons, 0, wxALIGN_CENTER_HORIZONTAL | wxBOTTOM, DIALOG_SPACING);
    SetSizer(main_sizer);
    main_sizer->SetSizeHints(this);
...
void WipingPanel::format_sizer(wxSizer* sizer, wxPanel* page, wxGridSizer* grid_sizer, const wxString& info, const wxString& table_title, int table_lshift/*=0*/)
{
    ...
    table_sizer->Add(grid_sizer, 0, wxALIGN_CENTER | wxTOP, DIALOG_SPACING);
}
...
: wxPanel(parent,wxID_ANY, wxDefaultPosition, wxDefaultSize/*,wxBORDER_RAISED*/)
{
    ...
    auto gridsizer_simple   = new wxGridSizer(3, SMALL_SPACING, GRID_VERTICAL_GAP);
    m_gridsizer_advanced = new wxGridSizer(m_number_of_extruders + 1, SMALL_SPACING, 1);
    ...
    for (unsigned int i = 0; i < m_number_of_extruders; ++i) {
        ...
        hsizer->AddSpacer(LARGE_SPACING);
    ...
    for (unsigned int i=0;i<m_number_of_extruders;++i) {
        add_spin_ctrl(m_old, extruders[EXTRUDER_PAIR_SIZE * i]);
        add_spin_ctrl(m_new, extruders[EXTRUDER_PAIR_SIZE * i + 1]);
        ...
        hsizer->AddSpacer(DIALOG_SPACING);
    ...
    m_page_advanced->Bind(wxEVT_PAINT,[this](wxPaintEvent&) {
                                              ...
                                              dc.DrawRotatedText(label,xpos-text_height,y_pos + text_width / HALF_DIVISOR,90);
    });
}
