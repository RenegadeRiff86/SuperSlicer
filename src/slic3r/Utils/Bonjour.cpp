...
struct DnsRR_AAAA
{
    ...
    static void decode(optional<DnsRR_AAAA> &result, const DnsResource &rr)
    {
        ...
        if (rr.data.size() == ip.size()) {
            DnsRR_AAAA res;
            std::copy_n(rr.data.begin(), ip.size(), ip.begin());
...
struct DnsMessage
{
    ...
    static optional<DnsMessage> decode(const std::vector<char>& buffer, const Bonjour::TxtKeys& txt_keys)
    {
        ...
        constexpr size_t minimum_message_size =
            static_cast<size_t>(DnsHeader::SIZE) + static_cast<size_t>(DnsQuestion::MIN_SIZE);
        if (size < minimum_message_size || size > MAX_SIZE) {
            return std::nullopt;
        }
