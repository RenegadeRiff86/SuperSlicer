...
static constexpr int WINRT_BLOCKING_TICK_MS = 100;
static constexpr unsigned PERCENT_COMPLETE = 100;
...
template<typename T>
static AsyncStatus winrt_async_await(const Microsoft::WRL::ComPtr<T> &asyncAction, ThrowOnCancelFn throw_on_cancel, int blocking_tick_ms = WINRT_BLOCKING_TICK_MS)
{
...
bool fix_model_by_repair_gui(ModelObject &model_object, int volume_idx, wxProgressDialog& progress_dialog, const wxString& msg_header, std::string& fix_result)
{
    ...
    auto on_progress = [&mtx, &condition, &ivolume, &volumes, &progress](const char *msg, unsigned prcnt) {
        ...
        progress.percent = static_cast<int>(floor((float(prcnt) + float(ivolume) * float(PERCENT_COMPLETE)) / float(volumes.size())));
    ...
    auto worker_thread = boost::thread([&model_object, &volumes, &ivolume, on_progress, &success, &canceled, &finished]() {
        try {
            ...
            -- ivolume;
            on_progress(L("Model repair finished"), PERCENT_COMPLETE);
            success  = true;
            finished = true;
        } catch (RepairCanceledException & /* ex */) {
            canceled = true;
            finished = true;
            on_progress(L("Model repair canceled"), PERCENT_COMPLETE);
        } catch (std::exception &ex) {
            success = false;
            finished = true;
            on_progress(ex.what(), PERCENT_COMPLETE);
        }
    });
    while (! finished) {
        std::unique_lock<std::mutex> lock(mtx);
