...
private:
    struct Data
    {
        explicit Data(std::string input_data) : data(std::move(input_data)) {}

        // Reference counter for the non-atomic intrusive owner below. Undo history is
        // single-threaded, so this avoids shared_ptr's atomic refcount traffic.
        size_t      refcnt{1};
        std::string data;

        bool matches(const std::string& rhs) const { return data == rhs; }

        bool matches_timestamp(uint64_t timestamp) const
        {
            assert(timestamp > 0);
            assert(data.size() > sizeof(timestamp));
            return memcmp(data.data(), &timestamp, sizeof(timestamp)) == 0;
        }
    };
    ...
    public:
        DataPtr() = default;
        explicit DataPtr(std::unique_ptr<Data> data) : m_data(data.release()) {}
        DataPtr(const DataPtr &other) : m_data(other.m_data) { add_ref(); }
        DataPtr(DataPtr &&other) noexcept : m_data(std::exchange(other.m_data, nullptr)) {}
        ...
        Data* operator->() const { return m_data; }
...
public:
    MutableHistoryInterval(const Interval &interval, const std::string &input_data)
        : m_interval(interval), m_data(std::make_unique<Data>(input_data)) {}

    MutableHistoryInterval(const Interval &interval, MutableHistoryInterval &other)
        : m_interval(interval), m_data(other.m_data) {}

    // as a key for std::lower_bound
    MutableHistoryInterval(const size_t begin, const size_t end) : m_interval(begin, end) {}

    MutableHistoryInterval(MutableHistoryInterval&&) noexcept = default;
    MutableHistoryInterval& operator=(MutableHistoryInterval&&) noexcept = default;
    ...
    const char* data() const { return m_data->data.data(); }
    size_t size() const { return m_data->data.size(); }
    ...
    size_t memsize() const {
        const size_t data_size = m_data->data.size();
        return m_data->refcnt == 1 ?
            ...
            (data_size + m_data->refcnt - 1) / m_data->refcnt;
    }
...
{
    std::unique_ptr<T> target = std::make_unique<T>(DeserializationToken{});
