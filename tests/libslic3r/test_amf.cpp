...
SCENARIO("Reading deflated AMF files", "[AMF]") {
    ...
    GIVEN("Compressed AMF file of a 20mm cube") {
        auto model {std::make_unique<Slic3r::Model>()};
        WHEN("file is read") {
            bool result_code = load_amf(get_model_path("test_amf/20mmbox_deflated.amf"s).c_str(), &_tmp_config, &_tmp_conf_sub, model.get(), false);
        ...
        WHEN("single file is read with some subdirectories") {
            bool result_code = load_amf(get_model_path("test_amf/20mmbox_deflated-in_directories.amf"s).c_str(), &_tmp_config, &_tmp_conf_sub, model.get(), false);
        ...
        WHEN("file is read with multiple files in the archive") {
            bool result_code = load_amf(get_model_path("test_amf/20mmbox_deflated-mult_files.amf"s).c_str(), &_tmp_config, &_tmp_conf_sub, model.get(), false);
    ...
    GIVEN("Uncompressed AMF file of a 20mm cube") {
        auto model {std::make_unique<Slic3r::Model>()};
        WHEN("file is read") {
            bool result_code = load_amf(get_model_path("test_amf/20mmbox.amf"s).c_str(), &_tmp_config, &_tmp_conf_sub, model.get(), false);
            THEN("Does not return false.") {
                REQUIRE(result_code == true);
            }
            THEN("Model object contains a single ModelObject.") {
        ...
        WHEN("nonexistant file is read") {
            bool result_code = load_amf(get_model_path("test_amf/20mmbox-doesnotexist.amf"s).c_str(), &_tmp_config, &_tmp_conf_sub, model.get(), false);
            THEN("Read returns false.") {
...
SCENARIO("Reading AMF file", "[AMF]") {
    ...
    GIVEN("badly formed AMF file (missing vertices)") {
        ...
        WHEN("AMF model is read") {
            auto ret = Slic3r::load_amf(get_model_path("test_amf/5061-malicious.amf").c_str(), &_tmp_config, &_tmp_conf_sub, model.get(), false);
    ...
    GIVEN("Ok formed AMF file") {
        ...
        WHEN("AMF model is read") {
            ...
            auto ret = Slic3r::load_amf(get_model_path("test_amf/read-amf.amf").c_str(), &_tmp_config, &_tmp_conf_sub, model.get(), false);
