...
SCENARIO("lift() and unlift() behavior with large values of Z", "[!shouldfail]") {
    GIVEN("A config from a file and a single extruder.") {
        ...
        config.set_defaults();
        config.load(std::string{TEST_DATA_DIR} + "/" +"fff_print_tests/test_gcodewriter/config_lift_unlift.ini"s, Slic3r::ForwardCompatibilitySubstitutionRule::Disable);
        assert(!config.retract_lift.get_values().empty() && config.retract_lift.get_values().front() > 0);
...
SCENARIO("set_speed emits values with fixed-point output.") {
    ...
    GIVEN("GCodeWriter instance") {
        ...
        //        REQUIRE_THAT(writer.set_speed_mm_s(1.09321e+06), Catch::Equals("G1 F1093210.000\n"));
        //    }
        //}
        WHEN("set_speed is called to set speed to 9.99321e+04") {
            THEN("Output string is G1 F5995926") {
                REQUIRE_THAT(writer.set_speed_mm_s(9.99321e+04), Catch::Equals("G1 F5995926\n"));
            }
        }
        WHEN("set_speed is called to set speed to 1") {
            THEN("Output string is G1 F60") {
                REQUIRE_THAT(writer.set_speed_mm_s(1.0), Catch::Equals("G1 F60\n"));
            }
        }
        WHEN("set_speed is called to set speed to 203.2000022") {
            THEN("Output string is G1 F12192") {
                REQUIRE_THAT(writer.set_speed_mm_s(203.2000022), Catch::Equals("G1 F12192\n"));
            }
        }
        WHEN("set_speed is called to set speed to 203.2000522") {
            THEN("Output string is G1 F12192.003") {
                REQUIRE_THAT(writer.set_speed_mm_s(203.2000522), Catch::Equals("G1 F12192.003\n"));
            }
        }
    }
}
